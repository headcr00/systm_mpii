/**
 * @file shv_modbus.h
 *
 * @date 1 февр. 2018 г.
 * @author Shvabenland
 * @brief
 */

#ifndef SHV_MODBUS_H_
#define SHV_MODBUS_H_

#include "stm32f1xx.h"
#include "modbus_regmap.h"


extern TIM_HandleTypeDef htim3;
extern UART_HandleTypeDef huart1;


enum {
	TCP,
	RTU
};
struct ip {uint8_t ip1; uint8_t ip2; uint8_t ip3; uint8_t ip4;};
typedef struct{
	uint16_t id;
	uint8_t type;
	USART_TypeDef* USARTx;
	TIM_TypeDef * tim;
	ModbusRegPool * pool_of_registers;
	struct ip ip_addr;
	struct ip gw_ip_addr;
	struct ip dns_ip_addr;
	struct ip netmask_addr;
	uint16_t port;
	uint8_t busy;
}modbus_t;

extern modbus_t mb;

enum mb_error_code_e
{
    MB_ENOERR,                  /*!< no error. */
    MB_ENOREG,                  /*!< illegal register address. */
    MB_EINVAL,                  /*!< illegal argument. */
    MB_EPORTERR,                /*!< porting layer error. */
    MB_ENORES,                  /*!< insufficient resources. */
    MB_EIO,                     /*!< I/O error. */
    MB_EILLSTATE,               /*!< protocol stack in illegal state. */
    MB_ETIMEDOUT,               /*!< timeout error occurred. */
	MB_ITSNOTFORUS,				/*!< message not for us*/
	MB_CRCERR					/*!< CRC error */
};

typedef enum mb_error_code_e mb_error_code;

static const unsigned char table_crc_hi[] = {
    
};

static const unsigned char table_crc_lo[] = {
    
};

void TIM3_Update_Handler();

void modbus_init_registers();
mb_error_code modbus_rt_process_request(modbus_t * mb, char * input_buffer, char ** output_buffer, uint16_t * len);
mb_error_code modbus_tcp_process_request(modbus_t * mb, char * input_buffer, char ** output_buffer, uint16_t * len);
//PORTS
void init_uart1(modbus_t * mb_pointer);
void reinit_uart1(modbus_t * mb_pointer);
void modbus_init_w5500(modbus_t * mb);
void modbus_reinit_w5500(modbus_t * mb);
//MISC
void shv_modbus_fill_parameters_from_spi();
#endif /* SHV_MODBUS_H_ */
